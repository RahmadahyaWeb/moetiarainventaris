    ...
    <div class="row">
        <div class="col-md-6 col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="formEdit"
                        action="{{ route('items.update', ['kode_barang' => $item->kode_barang, 'code_id' => $item->code_id]) }}"
                        method="POST">
                        @csrf
                        @method('put')
                        <div class="row gap-3">
                            ...
                            <div class="col-12">
                                ...
                                <select id="priority" name="priority"
                                    ...
                                    <option value="low" @selected(old('priority', $item->priority) == 'low')>
                                        Low
                                    </option>
                                    <option value="high" @selected(old('priority', $item->priority) == 'high')>
                                        High
                                    </option>
                                </select>
                                @error('priority')
                            ...
                            <div class="col-12">
                                ...
                                <input type="number" id="minimum"
                                    ...
                                    placeholder="Enter Minimum Stock" value="{{ old('minimum', $item->minimum) }}">
                                @error('minimum')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="col-12">
                                ...
                                <select name="unit_id" id="unit_id"
                                    ...
                                    @foreach ($units as $unit)
                                        <option value="{{ $unit->id }}" @selected($unit->id == old('unit_id', $item->unit_id))>{{ $unit->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('unit_id')
